...
이번 작업의 목적은 **실제 배포될 퍼플팀 보안 관리자 대시보드**가 AI Pack에서 생성한 최신 보안 분석 결과를 PostgreSQL 기반 운영 API로 표시할 수 있게 만드는 것입니다.

이번 작업은 로컬 시연용 UI가 아닙니다. 퍼플팀 작업자가 백엔드와 DB도 함께 구축한다고 가정합니다. AI Pack은 `argos-security`에서 evidence를 분석하고 DB 적재용 export를 생성하는 producer이며, 운영 프론트는 AI Pack 파일이나 AI Pack API를 직접 최종 의존성으로 사용하지 않습니다.
...
argos-security AI Pack
  ...
  -> /opt/argos/security/ai_runtime/dashboard_api/latest/dashboard-db-ingestion-export.json 생성
  -> 내부 central_api 제공
     GET  /api/exports/db-ingestion
     GET  /api/scans/{scan_id}/exports/db-ingestion

Purple Team Dashboard Backend
  -> AI Pack DB ingestion export 수집
  -> PostgreSQL upsert
  -> 운영 API 제공
     GET  /api/scans
...
Purple Team Dashboard Frontend
  -> Purple Team Dashboard Backend API 호출
  -> 보안 관리자 화면에 표시
```
...
AI 설계/AI Pack:
  evidence 읽기, AI1/AI2 실행, 자산 registry 갱신, dashboard payload 생성, DB ingestion export 생성

퍼플팀 대시보드 백엔드:
  AI Pack export를 PostgreSQL에 적재하고 운영 API 제공

퍼플팀 대시보드 프론트:
  퍼플팀 백엔드 API 응답을 기존 화면 구조에 렌더링
```
...
운영 환경에서 AI Pack은 `argos-security` 중앙 시스템에서 실행되고, 퍼플팀 백엔드가 PostgreSQL에 적재할 수 있는 payload와 DB ingestion export를 생성합니다.

프론트는 실제 서버 파일 경로를 직접 읽지 않습니다. 운영에서는 퍼플팀 백엔드가 아래 payload를 DB에 적재한 뒤 `GET /api/scans`, `GET /api/scans/{scan_id}/details` 형태로 제공합니다.
...
AI Pack은 파일을 생성하지만, 운영 대시보드는 파일 기반으로 직접 동작하지 않습니다. 퍼플팀 백엔드가 아래 export를 수집해 PostgreSQL에 적재합니다.

```text
AI Pack output:
  /opt/argos/security/ai_runtime/dashboard_api/latest/dashboard-db-ingestion-export.json
...
PurpleTeam Backend:
  dashboard-db-ingestion-export.json
    -> PostgreSQL upsert
    -> 운영 /api/scans, /api/scans/{scan_id}/details 제공
```
...
```text
raw evidence JSONL
Wazuh / normalizer 원천 로그
AI Pack run snapshot
AI1/AI2 LLM context/debug metadata
대용량 evidence pack
AI3 report input artifact
PDF/Markdown report artifact
재처리 가능한 full dashboard payload snapshot
```

중요: DB의 `evidence_events`에는 원문 evidence 전체를 넣지 마세요. `evidence_id`, `event_type`, `trace_id`, `request_id`, `producer_vm`, `source_ref`, `summary` 같은 조회용 참조만 저장하세요. raw token, secret, password, private key, 개인정보 원문은 DB/API/UI에 저장하거나 표시하지 마세요.
...
퍼플팀 백엔드가 화면 응답으로 가공해야 하는 것은 위 state 파일이 아니라 아래 dashboard payload와 DB ingestion export입니다. 프론트는 백엔드 API 응답만 사용합니다.
...
## 3.1 실배포 API/DB 연동 지시

운영 프론트는 AI Pack API를 직접 호출하지 않습니다. 퍼플팀 백엔드 API를 호출하세요.

목표:

```text
AI Pack dashboard-db-ingestion-export.json
  -> PurpleTeam Backend ingest
  -> PostgreSQL
  -> PurpleTeam Backend /api/scans, /api/scans/{scan_id}/details 응답
...
```text
VITE_API_BASE_URL=https://<purpleteam-dashboard-backend>/api
```

개발/스테이징에서 AI Pack을 직접 확인해야 할 때만 별도 fallback base URL을 둘 수 있습니다.

```text
VITE_AI_PACK_API_BASE_URL=http://<argos-security-host>:8000/api
```
...
POST /api/scans/trigger
  퍼플팀 백엔드가 권한 확인 후 AI Pack 실행 또는 ingest job을 트리거한다.

POST /api/scans/{scan_id}/pentest
  Red Team 검증 결과를 퍼플팀 DB의 pentest_* 테이블에 저장한다.
  chain_scenario_id, step_id, evidence_ids, affected_asset_ids를 반드시 보존한다.

GET /api/health
  퍼플팀 백엔드, PostgreSQL, 마지막 AI Pack ingest 상태를 표시한다.
```

퍼플팀 백엔드 작업 범위:

```text
1. AI Pack 내부 endpoint 또는 파일에서 dashboard-db-ingestion-export 수집
   GET /api/exports/db-ingestion
   GET /api/scans/{scan_id}/exports/db-ingestion

2. export.tables.*를 PostgreSQL에 upsert

3. 프론트가 기존처럼 호출할 운영 API 제공
   GET  /api/scans
   GET  /api/scans/{scan_id}/details
   POST /api/scans/trigger
   POST /api/scans/{scan_id}/pentest
...
AI Pack `central_api` 직접 호출은 스테이징/내부 검증용입니다. 운영 브라우저가 장기적으로 직접 의존하면 안 됩니다.

에러 처리:

```text
퍼플팀 백엔드 API 또는 AI Pack ingest 연결 실패:
...
프론트 adapter를 구현하세요. 운영에서는 퍼플팀 백엔드가 PostgreSQL에서 scan/detail API를 제공합니다. AI Pack 파일 해석과 DB ingestion export 처리는 프론트가 아니라 퍼플팀 백엔드의 책임입니다.

프론트 adapter:

```text
PurpleTeam Backend /api/scans/{scan_id}/details response
  -> component props
```
...
```text
PurpleTeam Backend API response
  -> 기존 scan list item
...
7. DB ingestion export 자체를 UI에서 직접 렌더링하지 말고, 백엔드가 제공한 scan/detail 응답을 사용
```
...
2. npm run lint 성공
3. npm run build 성공
4. 퍼플팀 백엔드 /api/health 응답에서 PostgreSQL과 AI Pack ingest 상태 확인
5. 퍼플팀 백엔드가 AI Pack /api/exports/db-ingestion 또는 dashboard-db-ingestion-export.json을 PostgreSQL에 upsert
6. 퍼플팀 백엔드 /api/scans 응답을 scanService 또는 API client에서 호출
7. 퍼플팀 백엔드 /api/scans/{scan_id}/details 응답을 화면 detail shape로 변환
8. 기존 scan/detail 화면 흐름 유지
9. PostgreSQL에 적재된 실제 AI Pack 결과로 화면 렌더링 가능
...
퍼플팀 백엔드 API base URL 설정 위치
AI Pack DB ingestion export와 기존 프론트 필드 매핑표
PostgreSQL ingest 및 퍼플팀 백엔드 API 호출 테스트 결과
frontend 빌드/테스트 결과
아직 AI Pack export 또는 퍼플팀 백엔드 API가 제공해야 하는 필드
```
